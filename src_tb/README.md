...
2. The testbench `tb_multiple_sign_verify.sv` runs a predefined number (DEFAULT_NUM_RUNS) of **sign/verify** pairs by randomizing `sk_seed`, `pk_seed`, `salt`, and `root_seed` internally for each run. Once all runs have completed, the median & average latency for **sign** and **verify** is printed.
3. The testbench `tb_keygen_100.sv` runs 100 predetermined **keygen** runs and compares their output (public vector **y**) against the outputs generated by the Reference C implementation using the same input seeds.
4. The testbench `tb_sign_verify_50_det.sv` runs 50 predetermined back-to-back **sign/verify** runs and compares their output against the generated by the Reference C implementation using the same input seeds. This testbench also calculates median & average latency for both **sign** and **verify**.
